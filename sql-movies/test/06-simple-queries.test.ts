...
//${Math.round(parseFloat(`SUM(revenue_adjusted)`)*100)/100} as total_revenue
  it(
  ...
  it(
    ...
    async done => {
      ...
      FROM MOVIES
      WHERE budget > 100000000 AND YEAR > '2008'`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      ...
      FROM MOVIES
      WHERE STRFTIME('%Y', release_date) > '2008'
  ...
  it(
    ...
    async done => {
      const query = `SELECT homepage, COUNT(*) AS count FROM MOVIES
